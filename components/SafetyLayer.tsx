...
export const SafetyLayer: React.FC<SafetyLayerProps> = ({ onClose }) => {
  ...
  return (
    <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[200] bg-slate-900/40 backdrop-blur-md flex items-end sm:items-center justify-center sm:p-6"
    >
        <motion.div
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            ...
            className="w-full max-w-sm bg-white rounded-t-[32px] sm:rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
        >
